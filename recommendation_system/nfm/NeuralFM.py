...
from recommendation_system.nfm import util as DATA
...
def parse_args():
    ...
    parser.add_argument('--keep_prob', nargs='?', default='[0.8,0.5]',
                        help='Keep probability(i.e., 1-dropout_ratio) for each deep layer and the Bi-Interaction layer.'
                             '1: no dropout. Note that the last index is for the Bi-Interaction layer.')
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "Neural FM: dataset=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d,"
            " batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
